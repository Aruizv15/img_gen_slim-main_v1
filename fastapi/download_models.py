import os
from huggingface_hub import hf_hub_download, list_repo_files

HF_TOKEN = os.getenv("HF_TOKEN", "")
repo = 'novafem54/batchapp-models'
extensions = ('.safetensors', '.pt', '.pth', '.onnx', '.bin')
files = [f for f in list_repo_files(repo, token=HF_TOKEN) if f.endswith(extensions)]
print(f'Descargando {len(files)} modelos...', flush=True)

for f in files:
    print(f'Descargando: {f}', flush=True)
    # Quitar el prefijo 'volumes/models/' de la ruta
    relative_path = f.replace('volumes/models/', '', 1)
    local_path = os.path.join('/workspace/ComfyUI_app/models', relative_path)
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    hf_hub_download(
        repo_id=repo,
        filename=f,
        local_dir='/workspace/ComfyUI_app/models',
        local_dir_use_symlinks=False,
        token=HF_TOKEN
    )
    # Mover al lugar correcto si es necesario
    downloaded_path = os.path.join('/workspace/ComfyUI_app/models', f)
    if downloaded_path != local_path and os.path.exists(downloaded_path):
        os.makedirs(os.path.dirname(local_path), exist_ok=True)
        os.rename(downloaded_path, local_path)
        print(f'Movido a: {local_path}', flush=True)
    print(f'OK: {f}', flush=True)

print('Todos los modelos descargados', flush=True)
